 ...
 *
 * ✅ INSIGHTS REMOVED:
 * - Removed RSS/Insights-specific helpers (no rss-sync wrapper, no insights-specific API)
 * - Core realtime remains for listings/rates/etc.
 *
 * NOTE:
...
(() => {
  ...
  LARMAH.openWhatsApp = function (text) {
    ...
    const url = `${baseUrl}?phone=${encodeURIComponent(
      LARMAH.businessPhone
    )}&text=${encodeURIComponent(String(text || ""))}`;
    window.open(url, "_blank", "noopener,noreferrer");
  };

  LARMAH.openEmail = function ({ subject = "", body = "" } = {}) {
    const to = LARMAH.businessEmail || "business@heylarmah.tech";
  ...
  LARMAH.getSession = async function ({ force = false } = {}) {
    ...
    if (
      !force &&
      LARMAH.__sessionCacheTs &&
      now() - LARMAH.__sessionCacheTs < LARMAH.__sessionCacheTtlMs
    ) {
  ...
  LARMAH.submitRequest = async function ({
    header,
    category,
    fields,
    refPrefix = "WEB",
  ...
  // ---------- EDGE FUNCTIONS ----------
  // ✅ Kept generic invoke helper, but removed any Insights/RSS-specific wrappers.
  ...
  LARMAH.realtime.statusBadge = function (elId, status) {
    ...
    if (status === "SUBSCRIBED")
      ...
      el.innerHTML = `<i class="fa-solid fa-circle"></i> Live: ${LARMAH.escapeHtml(
        status
      )}`;
  };
  ...
  LARMAH.realtime.subscribe = function (table, onChange, name, options = {}) {
    ...
    const debounceMs =
      typeof options.debounceMs === "number" ? options.debounceMs : 650;
    ...
    if (options.filter && typeof options.filter === "string") {
      ...
      const parts = Object.entries(options.match)
        .filter(
          ([k, v]) =>
            k && v !== undefined && v !== null && String(v).trim() !== ""
        )
