...
-- =========================================================
-- New user self-registration enablement
-- Keeps public registration open while admin verification remains controlled.
-- =========================================================
create or replace function public.handle_new_user()
returns trigger as $$
declare
  requested_role text := coalesce(new.raw_user_meta_data ->> 'account_type', 'premium');
  admin_match boolean := lower(new.email) = lower(public.admin_email());
begin
  ...
  values (
    ...
    new.email,
    case
      when admin_match then 'admin'
      when requested_role in ('user','premium') then requested_role
      else 'premium'
    end,
    coalesce(new.raw_user_meta_data ->> 'full_name',''),
    ...
    case when admin_match then true else false end,
...
-- Keep admins able to verify/edit users while newly registered clients can read their own profile.
drop policy if exists profiles_read_own on public.profiles;
create policy profiles_read_own on public.profiles for select using (auth.uid() = id);
...
-- =========================================================
-- Premium user biodata dashboard upgrade
-- Allows premium users to update name, phone, country and profile details.
-- =========================================================
...
begin
  ...
  insert into public.profiles (
    id, email, role, full_name, phone, company, country, address, preferred_pillar, bio, website,
    account_status, is_verified, verified_at, updated_at
  ) values (
    user_id,
    user_email,
    ...
    coalesce(profile_preferred_pillar,''),
    coalesce(profile_bio,''),
    coalesce(profile_website,''),
    case when admin_match then 'verified' else 'pending' end,
    admin_match,
    case when admin_match then now() else null end,
    now()
  ...
  set email = coalesce(nullif(excluded.email,''), public.profiles.email),
      ...
      preferred_pillar = coalesce(nullif(excluded.preferred_pillar,''), public.profiles.preferred_pillar),
...
grant execute on function public.sync_my_profile(text, text, text, text, text, text, text, text) to authenticated;
