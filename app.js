...
LARMAH.SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRyY2hqaWZ1ZnBzdnZsemdwYWl5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjYwOTMwNDEsImV4cCI6MjA4MTY2OTA0MX0.MLr1iCF4gjz0wnT1IFISCV9eJtnbq96_W_i7wAMOSbY";
...
LARMAH.renderAuthPill = async function(){
  ...
  if(!sess){
    mount.innerHTML = '<a class="pill" href="auth.html"><i class="fa-solid fa-right-to-bracket"></i> Sign in</a>' +
                      '<a class="pill primary" href="premium.html"><i class="fa-solid fa-star"></i> Premium</a>';
    return;
  }
  const email = sess.user?.email || "Signed in";
  mount.innerHTML = '<button class="pill" title="'+LARMAH.escapeHtml(email)+'"><i class="fa-solid fa-user"></i> '+LARMAH.escapeHtml(email)+'</button>' +
...
LARMAH.renderCatalog = async function(category, mountId, onEnquire){
  ...
  if(!items.length){
    mount.innerHTML = '<div class="notice"><strong>No items found.</strong> Admin can add items in Command Center.</div>';
    return;
  }
  mount.innerHTML =
    '<div class="grid">' +
    items.map(it=>{
      return '<div class="card">' +
        (it.image_url ? '<div class="thumb"><img src="'+LARMAH.escapeHtml(it.image_url)+'" alt=""></div>' : '') +
        '<h3>'+LARMAH.escapeHtml(it.title)+'</h3>' +
        '<p>'+LARMAH.escapeHtml(it.description || '')+'</p>' +
        (it.price ? '<div class="price">'+LARMAH.escapeHtml(it.price)+'</div>' : '') +
        '<div class="meta">Updated: '+new Date(it.updated_at||Date.now()).toLocaleDateString()+'</div>' +
        '<div class="actions"><button class="btn small primary" data-enq="'+it.id+'">Enquire</button></div>' +
      '</div>';
    }).join('') +
    '</div>';
...
LARMAH.fetchInsights = async function(limit){
  const sb = await LARMAH.initSupabase();
  if(!sb) return [];
  const { data, error } = await sb.from("insights_posts").select("id,title,body,created_at").order("created_at",{ascending:false}).limit(limit||12);
  if(error){ console.error(error); return []; }
  return data || [];
};
...
LARMAH.renderInsights = async function(mountId){
  ...
  if(!posts.length){
    mount.innerHTML = '<div class="notice"><strong>No posts yet.</strong> Admin can publish in Command Center.</div>';
    return;
  }
  mount.innerHTML =
    '<div class="grid">' +
...
LARMAH.subscribeInsights = async function(mountId){
  const sb = await LARMAH.initSupabase();
  if(!sb) return;
  sb.channel("insights_posts_changes")
...
LARMAH.loadCrypto = async function(mountId){
  ...
  if(!el) return;
  try{
    const res = await fetch("https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,tether&vs_currencies=ngn&include_last_updated_at=true", {cache:"no-store"});
